...
import { smartExtractJob } from '../services/gemini';
...
export const CreateJobModal: React.FC<CreateJobModalProps> = ({ isOpen, onClose, clients, onSave, tenant_id }) => {
  ...
  const [items, setItems] = useState<any[]>([{ description: 'Production Support', qty: 1, unitPrice: 0 }]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setShifts([]);
      setItems([{ description: 'Production Support', qty: 1, unitPrice: 0 }]);
      setBriefText('');
      setSelectedClientId('');
  ...
  const handleAiExtract = async () => {
    ...
    try {
      const data = await smartExtractJob(briefText);
      if (data) {
        setJobDetails(prev => ({
          ...prev,
        ...
        if (data.suggestedItems?.length > 0) {
          setItems(data.suggestedItems.map((it: any) => ({
      ...
      alert("AI was unable to parse this brief. Please fill in details manually.");
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Calculate Bounding Dates for Shift-Based Projects
      ...
      <div className="bg-white w-full max-w-2xl rounded-[32px] shadow-2xl border border-slate-200 animate-in zoom-in-95 duration-200 my-auto">
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => setIsAiMode(!isAiMode)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${isAiMode ? 'bg-indigo-600 text-white' : 'bg-white text-indigo-600 border-indigo-100'}`}>
              <i className="fa-solid fa-wand-magic-sparkles mr-1.5"></i> AI Extract
            </button>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8 max-h-[75vh] overflow-y-auto custom-scrollbar">
          {isAiMode ? (
            <div className="space-y-4 animate-in fade-in duration-300">
               <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Project Brief / Email Content</label>
               <textarea 
                  className="w-full h-40 p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-medium text-sm"
                  placeholder="Paste brief content here..."
                  value={briefText}
                  onChange={(e) => setBriefText(e.target.value)}
               />
               <button type="button" onClick={handleAiExtract} disabled={isExtracting || !briefText.trim()} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg transition-all disabled:opacity-50">
                 {isExtracting ? <i className="fa-solid fa-spinner animate-spin mr-2"></i> : <i className="fa-solid fa-bolt mr-2"></i>}
            ...
            <>
                ...
                <div className="col-span-2 p-6 bg-indigo-50/50 border border-indigo-100 rounded-[32px]">
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight">Toggle between span or discrete shifts</p>
                    </div>
                    <button 
                      type="button" 
                  ...
                  {jobDetails.schedulingType === SchedulingType.SHIFT_BASED ? (
                    <div className="space-y-4">
                      {shifts.map((s, idx) => (
                        <div key={idx} className="bg-white p-4 rounded-2xl border border-indigo-100 shadow-sm space-y-3 animate-in slide-in-from-left-1">
                          <div className="flex items-center justify-between">
                            <input className="px-3 py-2 bg-slate-50 rounded-lg text-xs font-black border-none outline-none flex-1 mr-2" placeholder="Install / Show" value={s.title || ''} onChange={e => handleShiftChange(idx, 'title', e.target.value)} />
                            <div className="flex items-center gap-2">
                              <label className="flex items-center gap-1 cursor-pointer">
                                <input type="checkbox" className="w-3 h-3 rounded" checked={s.isFullDay || false} onChange={e => handleShiftChange(idx, 'isFullDay', e.target.checked)} />
                                <span className="text-[9px] font-black text-slate-400 uppercase">Full Day</span>
                              </label>
                              <button type="button" onClick={() => handleRemoveShift(idx)} className="text-slate-300 hover:text-rose-500"><i className="fa-solid fa-trash-can text-[10px]"></i></button>
                            </div>
                          </div>
                      ...
                      <button type="button" onClick={handleAddShift} className="w-full py-3 border-2 border-dashed border-indigo-200 rounded-2xl text-[10px] font-black text-indigo-400 uppercase tracking-widest hover:bg-white transition-all">+ Add Shift</button>
                    </div>
                  ) : (
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <span className="text-[8px] font-black text-indigo-300 uppercase px-1">Start Date</span>
                        <input type="date" className="w-full px-4 py-3 bg-white border border-indigo-100 rounded-xl font-bold" value={jobDetails.startDate} onChange={e => setJobDetails({...jobDetails, startDate: e.target.value})} />
                      </div>
                      <div className="space-y-1">
                        <span className="text-[8px] font-black text-indigo-300 uppercase px-1">End Date</span>
                        <input type="date" className="w-full px-4 py-3 bg-white border border-indigo-100 rounded-xl font-bold" value={jobDetails.endDate} onChange={e => setJobDetails({...jobDetails, endDate: e.target.value})} />
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-50">
                 <div className="flex items-center justify-between px-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Deliverables & Recharge</label>
          ...
          <div className="flex justify-between items-center border-t border-slate-100 pt-8 sticky bottom-0 bg-white">
            <div className="flex flex-col">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Est. Total Valuation</p>
              <p className="text-3xl font-black text-indigo-600">£{totalRecharge.toLocaleString()}</p>
            </div>
            <button type="submit" disabled={isSaving} className="px-10 py-4 bg-slate-900 text-white rounded-[20px] font-black text-xs uppercase tracking-widest shadow-xl flex items-center gap-2 hover:bg-black transition-all disabled:opacity-50">
