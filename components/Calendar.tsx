...
import { getCalendarDays, formatDate as formatDateLabel } from "../utils";
import { formatGoogleCalendarSummary } from "../services/googleCalendar";
  ...
  const allCalendarEntries = useMemo(() => {
    ...
    (jobs || []).forEach((job) => {
      ...
      if (job.schedulingType === SchedulingType.SHIFT_BASED) {
        if ((job as any).shifts && (job as any).shifts.length > 0) {
          (job as any).shifts.forEach((shift: any) => {
            ...
            entries.push({
              id: shift.id,
              jobId: job.id,
              title: buildInternalTitle(job, clientName, shift.title || "Shift"),
      ...
      entries.push({
        id: job.id,
        jobId: job.id,
        title: buildInternalTitle(job, clientName),
