    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-20 px-4">
           ...
           <div className="bg-white w-full max-w-4xl rounded-[40px] shadow-2xl my-8 overflow-hidden">
              ...
              <div className="p-8 bg-slate-100 overflow-x-auto">
                <div ref={docRef} className="bg-white p-16 pb-32 border border-slate-100 min-h-[1120px] w-[800px] mx-auto text-slate-900 shadow-sm font-sans">
                   ...
                   {/* Table and rest omitted for brevity in XML block, assuming user wants component structure preserved */}
                   <div className="flex justify-center p-20 text-slate-200 font-black uppercase tracking-[1em]">Protocol Content Area</div>
                </div>
              </div>
           </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <form id="job-full-edit-form" className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm space-y-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-2">
                ...
                <input name="poNumber" defaultValue={job.poNumber} className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none" />
              </div>
              <div className="space-y-2">
            ...
            <div className="pt-10 border-t border-slate-100 space-y-8">
               <div className="flex items-center justify-between">
                  ...
                  <button 
                    type="button" 
                    onClick={toggleSync}
                    className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase transition-all border shadow-lg ${!job.syncToCalendar ? 'bg-rose-600 text-white border-rose-600' : 'bg-white text-indigo-600 border-indigo-100'}`}
                  >
               ...
               {job.schedulingType === SchedulingType.SHIFT_BASED ? (
                 <div className="space-y-4">
                    {shifts.map((s, idx) => (
                      <div key={s.id} className="grid grid-cols-1 md:grid-cols-3 gap-4 p-5 bg-slate-50 rounded-[28px] border border-slate-200">
            ...
            <div className="pt-10 border-t border-slate-100">
               <div className="flex items-center justify-between mb-6">
                  ...
                  <button type="button" onClick={() => setItems([...items, { id: generateId(), jobId: job.id, description: '', qty: 1, unitPrice: 0, rechargeAmount: 0, actualCost: 0 }])} className="text-[10px] font-black text-indigo-600 uppercase hover:underline">+ Add Entry</button>
               </div>
