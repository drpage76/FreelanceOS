    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-20 px-4">
        ...
        <div className="fixed inset-0 z-[200] flex items-start justify-center bg-slate-900/80 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-4xl rounded-[40px] shadow-2xl overflow-hidden my-8 border border-slate-200">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 sticky top-0 z-[210] backdrop-blur-md">
              <span className={`px-4 py-2 rounded-xl text-xs font-black border ${STATUS_COLORS[previewData.inv.status]}`}>Status: {previewData.inv.status}</span>
              <div className="flex gap-2">
                {previewData.inv.status === InvoiceStatus.DRAFT && (
                  <button onClick={() => handleSendInvoice(previewData.inv)} className="px-6 py-2 bg-indigo-600 text-white rounded-xl font-black text-xs shadow-lg">Issue Invoice</button>
                )}
                <button onClick={handleDownloadPDF} disabled={isProcessing === 'downloading'} className="px-6 py-2 bg-slate-900 text-white rounded-xl font-black text-xs shadow-lg flex items-center gap-2">
                  {isProcessing === 'downloading' ? <i className="fa-solid fa-spinner animate-spin"></i> : <i className="fa-solid fa-file-arrow-down"></i>} PDF
                </button>
                <button onClick={() => setPreviewData(null)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-white text-slate-900 hover:text-rose-500 border border-slate-200"><i className="fa-solid fa-xmark"></i></button>
              </div>
            </div>
            <div className="p-10 bg-slate-50 custom-scrollbar overflow-x-auto">
               <div ref={docRef} className="bg-white p-12 pb-32 border border-slate-100 min-h-[1120px] w-[794px] mx-auto shadow-sm text-slate-900">
                 <div className="flex justify-between items-start mb-20">
                    <div>
                      {state.user?.logoUrl ? (
                        ...
                        <div className="flex items-center gap-2 mb-6 text-3xl font-black">Freelance<span className="text-indigo-600">OS</span></div>
                      )}
                      <h1 className="text-5xl font-black tracking-tight">Invoice</h1>
                      <p className="text-slate-900 font-bold text-xs mt-2 tracking-widest">Ref: {previewData.inv.id}</p>
                    </div>
                    <div className="text-right">
                       ...
                       {state.user?.vatNumber && <p className="text-xs font-black text-slate-900 mt-2">{state.user?.taxName || 'VAT'}: {state.user.vatNumber}</p>}
                    </div>
                 </div>

                 <div className="grid grid-cols-2 gap-10 mb-20">
                    <div>
                       <p className="text-xs font-black text-slate-300 tracking-widest mb-2">Billed To</p>
                    ...
                    <div className="text-right">
                       <p className="text-xs font-black text-slate-300 tracking-widest mb-2">Invoice Details</p>
                       <div className="space-y-1">
                          ...
                          <div className="mt-4 pt-4 border-t border-slate-50 space-y-1 text-right">
                             ...
                             <p className="text-[11px] font-bold text-slate-500">Location: {previewData.job.location || 'TBD'}</p>
                             <p className="text-[11px] font-bold text-slate-500 tracking-widest">Period: {formatDate(previewData.job.startDate)} — {formatDate(previewData.job.endDate)}</p>
                             {previewData.job.poNumber && (
                               <p className="text-[11px] font-black text-indigo-600 tracking-widest mt-2 border-t border-indigo-50 pt-1 inline-block">Purchase Order: {previewData.job.poNumber}</p>
                             )}
                          </div>
                       </div>
                    </div>
                 ...
                 <table className="w-full mb-12">
                    <thead>
                       <tr className="border-b-2 border-slate-900">
                          <th className="py-4 text-left text-xs font-black tracking-widest">Description</th>
                          <th className="py-4 text-center text-xs font-black tracking-widest">Qty</th>
                          <th className="py-4 text-right text-xs font-black tracking-widest">Rate</th>
                          <th className="py-4 text-right text-xs font-black tracking-widest">Amount</th>
                       </tr>
                    </thead>
                 ...
                 <div className="flex justify-end pt-10 border-t-2 border-slate-900">
                    <div className="w-64 space-y-4">
                       <div className="flex justify-between items-center">
                          <span className="text-xs font-black text-slate-900 tracking-widest">Net Total</span>
                       ...
                       {state.user?.isVatRegistered && (
                         <div className="flex justify-between items-center pt-2 border-t border-slate-50">
                            <span className="text-xs font-black text-slate-900 tracking-widest">{state.user?.taxName || 'VAT'} ({state.user?.taxRate || 20}%)</span>
                       ...
                       <div className="flex justify-between items-center pt-4 border-t-2 border-slate-900">
                          <span className="text-xs font-black tracking-widest">Total Payable</span>
                 ...
                 <div className="mt-20 pt-10 border-t border-slate-100 grid grid-cols-2 gap-10">
                    <div>
                       <p className="text-[9px] font-black text-slate-900 tracking-widest mb-4">Remittance Details</p>
                       <div className="bg-slate-50 p-6 rounded-2xl font-mono text-[11px] text-slate-700 leading-relaxed border border-slate-100 shadow-inner">
                         ...
                         {!state.user?.accountName && !state.user?.accountNumber && <p className="text-slate-900">Configure banking in Workspace Settings.</p>}
                       </div>
                    </div>
                    <div>
                       <p className="text-[9px] font-black text-slate-900 tracking-widest mb-4">Notes</p>
                       <p className="text-[11px] text-slate-500 leading-relaxed font-medium">Thank you for your prompt settlement. Please quote invoice reference {previewData.inv.id} on all payments.</p>
                    </div>
                 </div>
               </div>
            </div>
      ...
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl min-w-[140px] shadow-sm"><p className="text-[9px] font-black text-emerald-600 tracking-widest mb-1">Settled Income</p><p className="text-xl font-black text-emerald-900">{formatCurrency(financialStats.totalPaid, state.user)}</p></div>
          <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-2xl min-w-[140px] shadow-sm"><p className="text-[9px] font-black text-indigo-600 tracking-widest mb-1">Accounts Receivable</p><p className="text-xl font-black text-indigo-900">{formatCurrency(financialStats.totalOutstanding, state.user)}</p></div>
        </div>
      </header>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {billableRecords.length === 0 ? (
                <tr><td colSpan={5} className="p-20 text-center text-slate-900 font-bold tracking-widest text-xs">No project history recorded</td></tr>
              ) : (
                billableRecords.map(record => {
                  ...
                  return (
                    <tr key={invoice?.id || job.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="p-6 font-black text-xs text-indigo-600">{invoice?.id || <span className="text-slate-300">Unissued</span>}</td>
                      <td className="p-6">
                      ...
                      <td className="p-6">
                        <span className={`px-4 py-1.5 rounded-full text-xs font-black border ${STATUS_COLORS[status] || 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                          {status}
                        </span>
                      </td>
                      ...
                      <td className="p-6 text-center">
                        {invoice ? (
                          <button onClick={() => handlePreview(invoice)} disabled={isProcessingRow} className="bg-slate-900 text-white px-5 py-2 rounded-xl text-xs font-black tracking-widest hover:bg-indigo-600 transition-all">Open Doc</button>
                        ) : (
                          <Link to={`/jobs/${job.id}`} className="bg-indigo-600 text-white px-5 py-2 rounded-xl text-xs font-black tracking-widest hover:bg-indigo-700 transition-all">Go to Workspace</Link>
                        )}
                      </td>
                    </tr>
                  )
